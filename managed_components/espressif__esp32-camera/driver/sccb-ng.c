...
//my custom code, not official!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#include "bsp_driver.h"
int install_device_flag=0;
...
//-----start-------------------------------------
    //my custom code!!! not official 
    if(install_device_flag==0)
    {
        install_device_flag=1;
    }
    //只初始化一下
    else 
    {
...
{ // sccb use an already initialized I2C port
    ...
    {
        //修改了源码！！！！！ my custom code!!! not official !!!!!
...
int SCCB_Deinit(void)
{
    ...
    for (uint8_t i = 0; i < device_count; i++)
    {
        ESP_LOGE(TAG,"mytest ----------in deinit rm i2c bus,step 1");
...
//------------------------------------------------------------------------------------------------------------------------
    ...
    ESP_LOGE(TAG,"mytest ----------in deinit rm i2c bus,step 2");
    return ESP_OK;
}
