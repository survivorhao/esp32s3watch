...
void * lv_mem_alloc(size_t size)
{
    ...
    if(alloc == NULL) {
        LV_LOG_USER("couldn't allocate memory (%lu bytes)", (unsigned long)size);
// #if LV_LOG_LEVEL <= LV_LOG_LEVEL_INFO

        lv_mem_monitor_t mon;
        lv_mem_monitor(&mon);
        // LV_LOG_USER("used: %6d (%3d %%), frag: %3d %%, biggest free: %6d",
        //             (int)(mon.total_size - mon.free_size), mon.used_pct, mon.frag_pct,
        //             (int)mon.free_biggest_size);

                LV_LOG_USER("total %" PRIu32 ", used: %6d (%3d %%), frag: %3d %%, biggest free: %6d",
                    mon.total_size, (int)(mon.total_size - mon.free_size), mon.used_pct, mon.frag_pct,
                    (int)mon.free_biggest_size);
// #endif
